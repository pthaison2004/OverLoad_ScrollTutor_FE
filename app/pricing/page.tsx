...
import { Check, ArrowLeft, Zap, Loader2, Sparkles, Trophy, Star, Wallet, AlertCircle, Clock, ShieldCheck, MessageCircle } from "lucide-react";
...
export default function PricingPage() {
  ...
  const renderUpgradeButton = (
    ...
    if (balance === null) {
      return (
        <button disabled className="w-full py-3 rounded-xl bg-slate-100 text-slate-400 font-bold text-xs flex items-center justify-center gap-2">
    ...
    if (balance >= price) {
      return (
        ...
        >
          ...
          {activePlan ? "Gia hạn" : label} - {price.toLocaleString("vi-VN")}đ
        </button>
      );
    }
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center text-center">
          <span className="font-bold text-xs">{activePlan ? "Gia hạn" : label}</span>
          <span className={`text-[9px] font-medium mt-0.5 ${activePlan ? "text-red-500/80" : "text-slate-500"}`}>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 relative overflow-hidden flex flex-col items-center py-12 px-4 select-none">
      ...
      <div className="w-full max-w-5xl flex justify-between items-center mb-10 z-10">
        <Link href="/" className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors text-sm font-semibold group">
      ...
      <div className="w-full max-w-3xl grid md:grid-cols-2 gap-6 mb-12 z-10 items-stretch">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 flex flex-col hover:shadow-md transition-all duration-300 group relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-bold text-slate-900">Gói Plus</h3>
              <p className="text-slate-400 text-[10px] mt-0.5">Trải nghiệm nâng cao</p>
            </div>
          ...
          <div className="mb-4 flex items-baseline gap-1">
            <span className="text-3xl font-extrabold text-slate-900">{plusMonthlyPrice.toLocaleString("vi-VN")}đ</span>
            <span className="text-slate-400 text-xs">/ tháng</span>
            {isStudentApproved && <span className="text-[11px] text-slate-400 line-through ml-1.5">69.000đ</span>}
          </div>
        ...
        <div className="bg-indigo-50/10 border-2 border-indigo-500 rounded-3xl p-6 flex flex-col relative shadow-sm hover:shadow-md transition-all duration-300 group overflow-hidden">
          ...
          <div className="mb-4 flex items-baseline gap-1">
            <span className="text-3xl font-extrabold text-indigo-600">{proMonthlyPrice.toLocaleString("vi-VN")}đ</span>
            <span className="text-slate-400 text-xs">/ tháng</span>
            {isStudentApproved && <span className="text-[11px] text-slate-400 line-through ml-1.5">119.000đ</span>}
          </div>
          <div className="space-y-3 mb-6 flex-1">
      ...
      {/* Trust Badges */}
      <div className="w-full max-w-5xl border-t border-slate-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-400 text-xs text-center md:text-left z-10">
        <div>
      ...
      {paymentResult && (
        <PaymentResultModal status={paymentResult} onClose={() => setPaymentResult(null)} />
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-[fadeIn_0.15s_ease-out]">
          <div className="bg-white text-slate-800 border border-slate-100 rounded-3xl w-full max-w-sm p-6 shadow-xl text-center select-none animate-[scaleIn_0.15s_ease-out]">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => router.push(`/deposit?amount=${showConfirmDeposit.amount}`)}
